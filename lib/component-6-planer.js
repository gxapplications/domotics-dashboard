...
// TODO !6: deplacer dans le component 6
class Planer {
  ...
  remove () {
    schedule.cancelJob(this.scheduledJob)
    console.log(`Heating dashboard planer - Scheduler removed for #${this.id}.`)
  }
  ...
  _changeMode (toComfort) {
    db.getComponentById(null, this.id, (err, component) => {
      if (err) {
        console.log(err)
        this._persistentStateUpdate('temp_dashboard_db_not_found_error')
        return
      }
      ...
      this._switchScenariiInSerial(toTurnOff, 'off')
        ...
        .catch(() => {
          ...
          this._persistentStateUpdate('temp_dashboard_planer_update_error')
        })
    })
  }
  ...
  _persistentStateUpdate (error = false) {
    ...
    dataStates[`domotic_heating_dashboard_${this.id}`] = {force_mode: this.forceMode, component: this.id, error: error}
