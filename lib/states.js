...
states.socketClient.connectedCallback = (err) => {
  if (err) {
...
states.socketClient.reconnectNow = () => {
  console.log('Web socket reconnection triggered...')
  states.socketClient.errorCount = 0
  states.socketClient.disconnect(
      () => {
...
states.init = () => {
  ...
  states.activableScenarii.load()
  states.socketClient.connect({retries: 8, delay: 2000, maxDelay: 8000}, states.socketClient.connectedCallback)
  window.addEventListener('online', states.socketClient.reconnectNow)
}
