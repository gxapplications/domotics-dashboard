...
import { fixPayload } from './model-middleware'
...
const actions = function (api, reply, page, component, action = null, payload = {}) {
  ...
  if (actionKey === 'inspect_temperatures') {
    ...
    const volatileConfiguration = fixPayload(payload)
    ...
    const {minmax, value} = fixPayload(payload)
    ...
    const newConfiguration = Object.assign({}, configuration, fixPayload(payload))
